...
$query = "SELECT denuncia.id_denuncia,folio,cat_opciones_sistemas.nombre AS sistema,clasificacion_denuncia.fecha_clasificacion AS fechaClasificacion,cat_tipo_denuncia.tipo_denuncia AS tipo,cat_estatus.estatus AS estatus,dependencialibre,narracion_hechos 
            ...
            WHERE NOT denuncia.estatus = 1 AND NOT denuncia.estatus = 3 AND NOT denuncia.estatus = 21
            ORDER BY denuncia.id_denuncia DESC LIMIT 30";
...
if ($resultado->num_rows >= 1){
        ...
        <small class="text-muted pt-1 folio" style="display:none;">Folio: <strong>'.$denuncias['folio'].'</strong></small>                
            ...
            <h6 class="mb-1">'.mb_strimwidth($denuncias['dependencialibre'],0,15,"...").' - '.mb_strimwidth($denuncias['tipo'],0,15,"...").'</h6>
                <div class="d-flex flex-column align-items-end">
                    <small class="text-muted pt-1">Fecha de clasificacion: '.$denuncias['fechaClasificacion'].'</small>                
                    <small class="text-muted pt-1 folio">Folio: <strong>'.$denuncias['folio'].'</strong></small>                
                    <small class="text-muted pt-1">Estatus: <strong class="badge rounded-pill bg-success text-white p-1">'.$denuncias['estatus'].'</strong></small>
                    <small class="text-muted pt-1">Sitema destino: <strong class="p-1">'.$denuncias['sistema'].'</strong></small>
                    <a href="revisarDenuncia.php?id='.$denuncias['id_denuncia'].'&ob=true">Ver denuncia</a>
                </div>
            </div>
    ...
    echo '<small class="text-center">No hay denuncias clasificadas</small>';
}